	...
	public List<List<String>> findDuplicate(String[] paths) {
		...
		for (String str:paths) {
			...
			for (int i=1; i<arr.length; i++) {
				int indexOfLeftBracket = arr[i].indexOf('(');
				String filename = arr[i].substring(0, indexOfLeftBracket);
				String content = arr[i].substring(indexOfLeftBracket+1, arr[i].length()-1);
